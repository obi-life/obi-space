...
import { 
  Sparkles, Palette, Activity, Users, Music, Camera, 
  PartyPopper, Trees, Building2, GraduationCap, Heart, Grid3x3
} from "lucide-react";
...
const Spaces = () => {
  ...
  const allItems: SpaceItem[] = [
    ...
    },
    {
      id: "yoga-wellness",
      title: "Yoga & Wellness",
      description: "Outdoor yoga sessions, meditation, and wellness programs in nature.",
      icon: Activity,
      ageGroups: ["Teens (13-17)", "Adults (18+)"],
      category: "movement",
      type: "program"
    ...
    },
    {
      id: "sports-games",
      title: "Sports & Games",
      description: "Organized sports activities, team games, and recreational activities.",
      icon: Users,
      ageGroups: ["Kids (7-12)", "Teens (13-17)", "Adults (18+)"],
      category: "movement",
      type: "program"
  ...
  const renderItem = (item: SpaceItem, index: number) => {
    if (item.type === "space" && item.image) {
      return (
        <div
          key={item.id}
          className={`group bg-card rounded-2xl overflow-hidden shadow-[var(--shadow-subtle)] transition-all duration-300 cursor-pointer animate-scale-in stagger-${(index % 4) + 1} ${contentAnimation.isVisible ? 'visible' : ''} hover:shadow-[var(--shadow-warm)] hover:-translate-y-2 hover:scale-[1.02]`}
        >
    ...
    const gradientClass = item.category === "movement" ? "gradient-nature" : "gradient-sunset";
    ...
    return (
      <div
        ...
        className={`group bg-card rounded-2xl overflow-hidden shadow-[var(--shadow-subtle)] transition-all duration-300 cursor-pointer animate-scale-in stagger-${(index % 4) + 1} ${contentAnimation.isVisible ? 'visible' : ''} hover:shadow-[var(--shadow-warm)] hover:-translate-y-2 hover:scale-[1.02]`}
      >
        <div className={`relative h-48 sm:h-52 overflow-hidden ${gradientClass} flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed">
              Discover the diverse range of spaces and programs we offer for events, creativity, movement, and community building.
            </p>
          </div>
        </div>
      </section>
            ...
            {filters.map((filter) => {
              ...
              return (
                <Button
                  ...
                  `}
                >
                  <IconComponent className={`w-4 h-4 sm:w-5 sm:h-5 mr-2 transition-colors duration-300 ${!isActive ? 'group-hover:text-primary' : ''}`} />
                  <span className={`text-sm sm:text-base transition-colors duration-300 ${!isActive ? 'group-hover:text-primary' : ''}`}>{filter.label}</span>
                  {!isActive && (
          ...
          >
            <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-black mb-1 text-center">
              {activeFilter === "all" ? "All Activities" : 
               activeFilter === "event-venue" ? "Event Venues" :
               activeFilter === "art" ? "Art Programs & Spaces" :
               activeFilter === "movement" ? "Movement & Fitness" :
               "Other Activities"}
            </h2>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-base sm:text-lg text-muted-foreground mb-6">
              Get in touch with us to book a space or learn more about our programs.
            </p>
